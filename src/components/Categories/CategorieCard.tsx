...
export const CategorieCard: React.FC<Props> = ({
  title,
  onExpandPress,
  onIconPress,
  children,
  ...
  return (
    <Pressable
      style={[styles.cardContainer, isSelected && styles.selected]}
      {...rest}
    >
      ...
      <Animated.View style={[styles.cardDescription, animatedStyle]}>
        <NunitoText customStyles={styles.cardDescription}>
          {description}
        </NunitoText>
        {expanded && (
          <AntDesign
            ...
            name="checkcircle"
            size={30}
            color={isSelected ? COLORS.successGreen : COLORS.grayText}
          />
        )}
      </Animated.View>
    </Pressable>
