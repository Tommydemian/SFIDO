...
type Navigation = NativeStackNavigationProp<AuthStackParams, keyof AuthStackParams>;

export const useFormHandler = (mode: Mode, navigation: Navigation ) => {
    // useForm hook
  const {control, handleSubmit, formState: {errors}, reset, watch, clearErrors} = useForm<FormData>({
    mode: mode && mode, 
  })
...
const emailRules = {
  ...
  return {passwordRules, emailRules, reset, watch, control, handleSubmit, clearErrors, setupFormNavigationCleanup}
}