...
export const MessageScreen: React.FC<Props> = ({ route }) => {
  ...
  useEffect(() => {
    const { receivedMessagesSubscriber, sentMessagesSubscriber } =
      handleSentAndReceivedMessages(id);

    return () => {
      receivedMessagesSubscriber(), sentMessagesSubscriber();
    };
  }, []);
