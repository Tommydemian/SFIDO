...
export const sendPasswordResetEmail = async (email: string) => {
  try {
    ...
    if (error instanceof Error && 'code' in error) {
      throw new Error(mapAuthErrorToMessage(error as FirebaseAuthTypes.NativeFirebaseAuthError));
    } else {
...
const mapAuthErrorToMessage = (error: FirebaseAuthTypes.NativeFirebaseAuthError): string => {
  switch (error.code) {
